...
int booking_function(int date, int month, int year, int i){
	...
	char choice[2];
	float price = final_price(date,month,year,weight,from,to,class_num);
	printf("\nThe Price of this ticket is ₹%d\n", price);
	...
	if (strcmp(choice,"n") == 0) return 0;
		...
		create_boarding_pass(name,phonnum,from,to, date,month,year,ttime,class_num,seat_no,price);
		return 1;
	}
}