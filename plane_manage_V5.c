...
void print_details(struct board b,int j){  
	printf("-------------------------------------------------------------------------------------------------------\n");
	printf(" %d | %12s | %s | %s | %d | %2d/%2d/%4d | %s | %8.2f |\n",j,b.name,b.from,b.to,b.ttime,b.date,b.month,b.year,b.class,b.price);
...
int asking_user(){  
	printf("if you need to cancel your ticket press 1\nif you need to see your boarding pass press 2\nto return home page press 0\n");
	int n;
	scanf("%d",&n);
	if (n==1){
		return 1;
